<template>
	  ...
	  <el-dialog v-model="paramDialogVisible" class="dialog" title="Paramètres" width="37%">
		...
		<el-tab-pane label="Général" name="first">
			...
			<el-form-item label="supprimer les fausse données de jeu">
					<el-button @click="removeFakeGameStats()">Supprimer</el-button>
			</el-form-item>
			</el-form>
		</el-tab-pane>
			...
			<el-form label-width="30%" :label-position="`left`" status-icon>
				...
				<el-form-item label="Pseudo">
					...
					<el-button @click="showPseudoInput = !showPseudoInput" v-if="!showPseudoInput">Modifier</el-button>
		 			...
		 			<el-button @click="showPseudoInput = !showPseudoInput" v-if="showPseudoInput"  v-model="form.username">annuler</el-button>
		  			<el-button @click="showPseudoInput = !showPseudoInput; updateUser()" v-if="showPseudoInput"  v-model="form.username" >valider</el-button>
				</el-form-item>
				<el-form-item label="Mot de passe">
					<el-alert v-if="loggedInUser?.isOauth" title="Les comptes Oauth ne peuvent pas avoir de mot de passe" type="warning" show-icon :closable="false" />
					...
					<el-input v-if="showPasswordInput" type="password" v-model="password" show-password placeholder="new password"></el-input>
					...
					<el-button @click="showPasswordInput = !showPasswordInput; updateMyPassword()" v-if="showPasswordInput" type="password" v-model="password">valider</el-button>
					</div>
				</el-form-item>
			</el-form>
          <!-- <div class="avatar-section">
		...
		<el-tab-pane label="2fa" name="third">
			<el-alert v-if="loggedInUser?.isOauth" title="Vous utilisez un compte Oauth, et n epouvez pas activer le 2fa" type="info" show-icon :closable="false" />
			<div v-if="!loggedInUser?.isOauth">
			<el-alert v-if="activateTwoFactorInput" title="rentrez le code obtenu par l'application google authentificator pour activer le 2fa" type="warning" :closable="false" />
  ...
  import { useFindMyUserQuery,
			...
			useInjectFalseGameStatDataMutation,
			useUpdateMyPasswordMutation,
			useRemoveFalseGameStatDataMutation
  ...
  const { mutate:removeFakeGameStatsmutate } = useRemoveFalseGameStatDataMutation()
...
{
	validategooglesecretmutate({args : {
		code :secret
	}}).then(
		(res) => {
...
const removeFakeGameStats = async () => {
	await removeFakeGameStatsmutate().then(() => {
		ElMessage.success(`Les fausses données de jeu ont bien été supprimées`)
		return true
	})
}
